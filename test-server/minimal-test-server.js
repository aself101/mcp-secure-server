// minimal-test-server.js - Uses SecureMcpServer for automatic transport-level security
        ...
        this.server = new SecureMcpServer(
            {
                ...
                version: "0.3.0",
                description: "Clean debug server using SecureMcpServer - automatic transport-level security"
            },
            {
                // Logging (opt-in)
                enableLogging: true,
        ...
        this.server.tool(
            ...
            async ({ path }) => {
                try {
                    ...
                    return { content: [{ type: "text", text: `File Error: ${error.message}` }] };
                }
            }
        );
        ...
        this.server.tool(
            ...
            async ({ data, format }) => {
                try {
                    ...
                    return { content: [{ type: "text", text: `Parser Error: ${error.message}` }] };
                }
            }
        );
