 ...
 * @param {Array<string>} strings - Extracted strings to validate
 * @returns {{ passed: boolean, reason?: string, severity?: string, violationType?: string, confidence?: number }}
 */
export function validateDataFormats(strings) {
 ...
 * @returns {{ passed: boolean, reason?: string, severity?: string, violationType?: string, confidence?: number }}
 */
export function validateEncodingConsistency(content) {
    const encodingCount = {
 ...
 * @returns {{ passed: boolean, reason?: string, severity?: string, violationType?: string, confidence?: number }}
 */
export function validateParameters(message) {
    ...
    const nestingLevel = calculateNestingLevel(message.params);
    if (nestingLevel > 15) {
        return {
            passed: false,
    ...
    let paramString;
    try {
        paramString = JSON.stringify(message.params);
    } catch (error) {
        return {
            passed: false,
            reason: `Parameter serialization error: ${error.message}`,
            severity: 'MEDIUM',
    ...
    const paramCount = countParameters(message.params);
    if (paramCount > 100) {
        return {
            passed: false,
 ...
 * @returns {{ passed: boolean, reason?: string, severity?: string, violationType?: string, confidence?: number }}
 */
export function validateContext(message, context) {
