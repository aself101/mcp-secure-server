...
export function normalizeUnicode(input) {
    ...
    const homoglyphMap = {
        // Cyrillic → Latin (the bypass you found)
        'а': 'a', 'е': 'e', 'о': 'o', 'р': 'p', 'с': 'c', 
        'х': 'x', 'у': 'y', 'і': 'i', 'ѕ': 's', 'т': 't',
...
export function decodeUnicodeEscapes(input) {
    ...
    decoded = decoded.replace(/\\\\x\{([0-9A-Fa-f]{1,6})\}/g, (_m, hex) => {
        try {
...
export function removePostDecodingZeroWidth(input) {
...
export const decodeEntities = (input) => {
    ...
    decoded = decoded.replace(/&#x0*([0-9A-Fa-f]+);?/gi, (match, hex) => {
        try {
            const codePoint = parseInt(hex, 16);
    ...
    decoded = decoded.replace(/&#0*(\d+);?/g, (match, dec) => {
        try {
 ...
 */
export const normalizeCJKCompatibility = (codePoint) => {
    const cjkMappings = {
        ...
        // Add more mappings as needed for evasion techniques
    };
